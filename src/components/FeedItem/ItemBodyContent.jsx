...
import styles from './ItemBodyContent.module.css'
import icoLike from '../../assets/ico_like.svg';
import icoReply from '../../assets/ico_reply.svg';
import icoShare from '../../assets/ico_share.svg';

export default function ItemBodyContent({ feedData }) {
	const renderTags = (tags) => {
		return tags.split(' ').map((tag, index) => (
			<li className={styles.tags_items} key={index}><a href={`https://vastyle.co.kr/shop_new/search_renew.php?stx=${tag.replace(/#/g, '')}&viewtab=style`} target='_blank' rel='noopener noreferrer'>{tag}</a></li>
		));
	};
	...
	return (
		<>
			...
			<div className={styles.util}>
				<button type='button' className={styles.btn_like}>
					<img src={icoLike} alt="좋아요" />
				</button>
